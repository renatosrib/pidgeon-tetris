...
function limparPecasEsquerda()
    for current in range(0, ALTURA_TELA, 1) do
        ...
        GAME_GRID_TABELA:set_cell(BORDA_PECAS - 20, current - 20, 0)
        GAME_GRID_TABELA:set_cell(BORDA_PECAS + 20, current + 40, 0)
        GAME_GRID_TABELA:set_cell(BORDA_PECAS + 20, current - 40, 0)
        GAME_GRID_TABELA:set_cell(BORDA_PECAS - 20, current - 40, 0)
        GAME_GRID_TABELA:set_cell(BORDA_PECAS - 40, current - 20, 0)
        GAME_GRID_TABELA:set_cell(BORDA_PECAS - 40, current - 40, 0)
    end
end
...
function love.load()
    ...
    for x in range(0, LARGURA_TELA / 2) do
        GAME_GRID_TABELA:set_cell(x, BORDA_PECAS, "_")
    end
    desenharPecasAcido()
end
...
function love.update(dt)
    ...
    if (vivo) then
            ...
            if (math.fmod(dt * 1000, 2) >= 0 and math.fmod(dt * 1000000, velocidade) <= 2 and pombo.status == pombostatus.descendo) then
                if (pombo.posy <= BORDA_PECAS + 1 - pombo_direita:getWidth()) then
                    ...
                    if (pombo.posx == BORDA_PECAS - pombo_direita:getWidth()) then
                        ...
                        posicao = -100
                        if (pombo.posx < 170) then
                            print("pombo.posy", pombo.posy)
                            posicao = 100
                        elseif (pombo.posx >= 170 and pombo.posy < 250) then
                            print('200')
                            posicao = 200
                        elseif (pombo.posx >= 250) then
                            print('300')
                            posicao = 300
                        end
                    ...
                    if (pombo.status == pombostatus.acido and pombo.peca == 0 or pombo.peca == nil) then
                        modo = SOLTANDO_PECAS
                        vivo = false
                    else
                        limparPecasEsquerda()
        ...
        if (modo == SOLTANDO_PECAS) then
            ...
            if (love.keyboard.isDown("space") and pombo.posx >= BORDA_CENTRAL) then
                if (pombo.posy < BORDA_CENTRAL + 100) then
                    desenhar(pombo.posx, pombo.posy, pombo.peca)
                end
                score = score + 100
                pombo.peca = 0
                pombo.status = pombostatus.voando
...
function love.draw()
    if (vivo) then
        som:play()
        love.graphics.print("Score "  .. tostring(score), 710, 20, 0, 1, 1)
        if (modo == SOLTANDO_PECAS) then
            love.graphics.print("Peça atual: ", 2, 5, 0, 1, 1)
            desenhar(40, 40, pombo.peca)
        end
